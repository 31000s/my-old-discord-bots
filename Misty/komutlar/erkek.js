...
module.exports.run = async (bot, message, args) => {
 ...
 if (!message.member.roles.has(teyitci.id)) return message.channel.send(teyitmsj);
  ...
  if (!teyitci) {
    const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setDescription(` ${emojiler} Teyit rolü ayarlanmamış! \`\` ${prefix}kayıt-yetkilisi @rol \`\` `)
 ...
 //if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(teyitmsj);
  ...
  let kullanıcı = message.mentions.members.first()
      ...
      .setColor("RANDOM")
      .setDescription(` ${emojiler} Yanlış kullanım! \`\` ${prefix}erkek @kişi <isim> <yaş> \`\` `)
  ...
  //
  let verol = await db.fetch(`erkekverolk_${message.guild.id}`); 
  ...
  if (!verol) {
    const embed = new Discord.RichEmbed()
         .setColor("RANDOM")
      .setDescription(` ${emojiler} Verilecek rol ayarlanmamış! \`\` ${prefix}erkek-rolü @rol \`\` `)
    message.channel.send(embed);
    return;
  }
    ...
    const embed = new Discord.RichEmbed()
       .setColor("RANDOM")
      .setDescription(` ${emojiler} Yanlış kullanım! Yaş belirtmelisin! \`\` ${prefix}erkek @kişi <isim> <yaş> \`\` `)
    message.channel.send(embed);
    return;
  }
          if (!isim) {
    const embed = new Discord.RichEmbed()
       .setColor("RANDOM")
      .setDescription(` ${emojiler} Yanlış kullanım! İsim yazmalısın! \`\` ${prefix}erkek @kişi <isim> <yaş> \`\` `)
    message.channel.send(embed);
    return;
  }
...
.setTimestamp()
 user.addRole(verol)
  user.removeRole(alrol)
   ...
   db.add(`kayiterkek_${message.author.id}`, + 1);
