...
let user = message.mentions.members.first()
  ...
  if (tagX === null || tagX === undefined){
    ...
    return message.channel.send(
      ...
      .setDescription(`${emojiler} Yanlış Kullanım! Kullanıcı etiketmelisin! \`${prefix}nick @kullanıcı [isim]\` `)
      .setColor('RED')
    )
  ...
  if(!isim)
    return message.channel.send(
      new Discord.RichEmbed()
      .setDescription(`${emojiler} Yanlış Kullanım! Nick belirtmelisin!`)
      .setColor('RED')
    )
  ...
  .addField('Yeni Adı:', `${tag} ${isim}`, true)
